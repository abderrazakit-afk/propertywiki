...
export default function ChatModal({ isOpen, onClose }: ChatModalProps) {
  const [messages, setMessages] = useState<Message[]>([
    {
      role: 'assistant',
      content: 'Hello! I\'m your PropertyWiki assistant. Ask me anything about Dubai real estate - property types, communities, buying process, or homeowner tips!'
    }
  ])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.status === 429 || data.limitReached) {
        ...
        setMessages(prev => [...prev, { role: 'assistant', content: data.error || 'Daily message limit reached. Please try again tomorrow.' }])
      } else if (data.error) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md h-[600px] max-h-[80vh] flex flex-col overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 bg-gradient-to-r from-primary-500 to-primary-600">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-white font-semibold">PropertyWiki Assistant</h3>
              <p className="text-white/80 text-xs">Ask about Dubai real estate</p>
            </div>
          </div>
          <button
            onClick={onClose}
          ...
          <div className="flex-1 flex items-center justify-center p-6">
            <form onSubmit={handleEmailSubmit} className="w-full space-y-4">
              <div className="text-center mb-4">
                <p className="text-gray-700 font-medium">Enter your email to start chatting</p>
                <p className="text-xs text-gray-400 mt-1">20 free messages per day</p>
              </div>
              <input
                ref={emailInputRef}
                type="email"
            ...
            <form onSubmit={handleSubmit} className="p-4 border-t border-gray-100 bg-white">
              <div className="flex items-center space-x-2">
                <input
                  ...
                  placeholder={limitReached ? 'Daily limit reached' : 'Ask about properties, communities...'}
